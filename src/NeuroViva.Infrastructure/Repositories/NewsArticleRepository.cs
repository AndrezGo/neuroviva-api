...
public sealed class NewsArticleRepository : INewsArticleRepository
{
    ...
    public NewsArticleRepository(NeuroVivaDbContext db) => _db = db;

    public async Task UpsertManyAsync(IReadOnlyList<NewsArticle> articles, CancellationToken ct = default)
    {
        if (articles.Count == 0)
            return;
        ...
        try
        {
            await _db.NewsArticles.AddRangeAsync(newOnes, ct);
    ...
    public async Task<IReadOnlyList<NewsArticle>> ListByDiseaseIdsAsync(
